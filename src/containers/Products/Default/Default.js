...
class Default extends Component{
    state={
        products:this.props.default,
        currentPage:1,
        postsPerPage:9,
    }
    priceHighToLowOnClickHandler=()=>{
        let newProducts=this.props.default.sort((a,b)=>{
    ...
    filterByPrice=(a,b)=>{
        let newProducts=this.props.default.filter(function (el) {
            return el.price >= a &&
                el.price<=b;
        });
        this.setState(prevState=>({
                ...
                <Container fluid>
                    <Row className="ml-0 mr-0 mt-3">
                        <Col xs={12} sm={12} md={12} lg={12} xl={12} className={[classes.pageHeading]}>
                            <h4>Products</h4>
                            <Button variant="danger" style={{cursor:'not-allowed'}} className="ml-auto" disabled>Add Product</Button>
                        </Col>
                    </Row>
                                    ...
                                    currentPosts.map( products => (
                                        <Col className={classes.productContainer}  xs={12} sm={12} md={4} lg={4} xl={4} id={products.price}>
                                            <div className={classes.product}>
                                                {products.src ?
                                                    <ListedProduct src={products.src} name={products.name} price={products.price} category='default'/>:
