...
class Default extends Component{
    ...
    priceHighToLowOnClickHandler=()=>{
        let newProducts=this.props.bags.sort((a,b)=>{
    ...
    filterByPrice=(a,b)=>{
        let newProducts=this.props.bags.filter(function (el) {
            return el.price >= a &&
                el.price<=b;
        });
        this.setState(prevState=>({
    ...
    render() {
        ...
        if(this.state.addedProduct ? currentPosts = this.props.bags.slice(indexOfFirstPost, indexOfLastPost):
                                    ...
                                    currentPosts.map( products => (
                                        <Col className={classes.productContainer}  xs={12} sm={12} md={4} lg={4} xl={4} id={products.price}>
                                            <div className={classes.product}>
                                                {products.src ?
                                                    <ListedProduct src={products.src} name={products.name} price={products.price} category='bags'/>:
